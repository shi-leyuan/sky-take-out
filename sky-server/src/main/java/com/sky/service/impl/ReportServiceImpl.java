...
public class ReportServiceImpl implements ReportService {
    ...
    public void exportBusinessData(HttpServletResponse response) {
        ...
        //通过Apache POI将数据写入Excel文件
        InputStream in = this.getClass().getClassLoader().getResourceAsStream("template/运营数据报表模板.xlsx"); //输入流对象
        try {
            //基于模板创建新的Excel文件
            XSSFWorkbook excel = new XSSFWorkbook(in);
            //填充数据
            XSSFSheet sheet = excel.getSheetAt(0); //第一页
            sheet.getRow(1).getCell(1).setCellValue("时间：" + dateBegin + "至" + dateEnd); //第一行第一列

            //第四行
            XSSFRow row = sheet.getRow(3);
            row.getCell(2).setCellValue(businessDataVO.getTurnover()); //第四行第三列
            row.getCell(4).setCellValue(businessDataVO.getOrderCompletionRate()); //第四行第五列
            row.getCell(6).setCellValue(businessDataVO.getNewUsers()); //第四行第七列

            //获得第五行
            row = sheet.getRow(4);
            row.getCell(2).setCellValue(businessDataVO.getValidOrderCount()); //第五行第三列
            row.getCell(4).setCellValue(businessDataVO.getUnitPrice()); //第五行第六列
